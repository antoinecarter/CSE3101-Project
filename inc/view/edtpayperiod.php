<?php
    ...
    if(isset($_POST['update_payp'])){
        $cred = $payperiodsModel->updatepayp();
    }else if (isset($_POST['delete_payp'])){
        $cred = $payperiodsModel->deletepayp();
    }
...
<div class = "edit-usr">
    ...
    <form method="post" action="">
        ...
        <?php } ?>
        <div>
                <button type="submit" name="update_payp">Apply Changes</button>
            <?php if($row['status'] != 'VERIFY'){ ?><a href="./Users/Registration/Delete?id="<?php echo $row['id']?>> <button style = "background-color:#eb0b4e;"  name="delete_payp"> Delete</button></a> <?php } ?>
